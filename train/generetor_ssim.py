...
def getHoursGridFromNPY(filepath, delta_hour, config_dict):  # 20200619
    ...
    for s in param_list:
        ...
        if s == 'RAINNC':
            ...
            # ssim algorithm choose 3 channels
            tmp = []
            for t in range(npy_grid.shape[0]):
                var_array = np.ones([npy_grid.shape[1], ], dtype=np.float64) * (-np.inf)
                for i in range(1, npy_grid.shape[1] - 1):
                    var_array[i] = np.var(npy_grid[t, i].flatten())
                idx = np.argmax(var_array)
                tmp.append(npy_grid[t, idx-1:idx+2])
            npy_grid = np.stack(tmp, axis=0)
