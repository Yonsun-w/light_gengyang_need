...
from layers.TransformerDecoder import TransformerDecoder as Transformer
...
class WRFInfo(nn.Module):
    def __init__(self, channels, config_dict):
        ...
        self.cnn = nn.Conv2d(16, 8, kernel_size=1, stride=1)
    ...
    def forward(self, wrf):
        ...
        # concat
        wrf_h_alltime = torch.cat([wrf, wrf_h_alltime_for, wrf_h_alltime_rev], dim=2)
        wrf_h_alltime2 = []
        for i in range(self.config_dict['ForecastHourNum']):
            wrf_h_alltime2.append(self.cnn(wrf_h_alltime[i]))
        wrf_h_alltime = torch.stack(wrf_h_alltime2, dim=0)
        return wrf_h_alltime, None


class DCNADSNet_lite_tf4_Model(nn.Module):
    def __init__(self, obs_tra_frames, obs_channels, wrf_tra_frames, wrf_channels, config_dict):
        super(DCNADSNet_lite_tf4_Model, self).__init__()
        self.config_dict = config_dict
    ...
    def forward(self, wrf, obs):
        ...
        c = self.encoder_c(c)
        for t in range(self.wrf_tra_frames):
            wrf_tf = self.transformer(wrf_info_h[t], h)
...
if __name__ == "__main__":
    ...
    model = DCNADSNet_lite_tf_Model(obs_tra_frames=3, obs_channels=1, wrf_tra_frames=12, wrf_channels=29, config_dict=config_dict)
    print('# generator parameters:', sum(param.numel() for param in model.parameters()))
    print('# generator parameters:', sum(param.numel() for param in model.wrf_info.parameters()))
