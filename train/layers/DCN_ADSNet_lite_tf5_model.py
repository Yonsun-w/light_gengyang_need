...
class DCNADSNet_lite_tf5_Model(nn.Module):
    def __init__(self, obs_tra_frames, obs_channels, wrf_tra_frames, wrf_channels, config_dict):
        ...
        self.dcn_tm = ModulatedDeformConvTM(8, 1, kernel_size=5, stride=1, padding=2)
    ...
    def forward(self, wrf, obs):
        ...
        for t in range(self.wrf_tra_frames):
            ...
            wrf_tf = self.transformer(his_enc, wrf_encoder[t])
            h, c = self.decoder_ConvLSTM(wrf_tf, h, c)
            pre = self.decoder_module(h)
            pre = self.dcn_tm(pre, t + 1)
            pre_frames[t] = pre
